...
mod tests {
    ...
    /// Spark (the mobile wallet) hand-maintains a TypeScript mirror of `TxPayload` and this
    /// signing scheme, in `client/src/services/helixTx.ts` — it has to build the exact same
    /// bytes to sign a valid Helix transaction from a phone. Nothing enforces the two stay in
    /// sync; this already fired for real on 2026-07-20, when a same-day domain-separation
    /// change here (`crypto_version` added to `TxPayload`, the `helix-tx-v1:` tag added to
    /// `signing_hash`) broke Spark's signing silently until someone noticed and re-derived it
    /// by hand from Rust test vectors.
    ///
    /// This is the loud version of that: a transaction built from fixed, hardcoded fields (same
    /// seed as `phrase_and_key_derivation_match_sparks_javascript_implementation` in
    /// `helix-cli`, for one recognizable fixture instead of two) pinned against its exact
    /// encoded payload bytes and final signing hash. Change *anything* that
    /// `TxPayload`/`signing_hash` touch — field order, a new field, the domain tag, the bincode
    /// config — and this fails immediately in CI, not silently in a mobile app days later.
    /// Updating the hardcoded hex below is not a bug fix on its own: it's a flag that
    /// `client/src/services/helixTx.ts` needs the matching update before this ships.
    #[test]
    fn signing_format_matches_the_pinned_vector_sparks_typescript_mirror_depends_on() {
        let seed: Vec<u8> = (0u8..32).collect();
        let keypair = KeyPair::from_mldsa_seed(&seed).unwrap();
        let from = Address::from_public_key(&keypair.public);
        assert_eq!(from.to_string(), "hlxZiWwobcPKCRx8qjZECjeitEufkor2NQ1S");
        ...
        let payload = bincode::serialize(&TxPayload {
            ...
            nonce: tx.nonce,
            data: &tx.data,
            crypto_version: tx.crypto_version,
        })
        .unwrap();
        assert_eq!(
            hex::encode(&payload),
            "",
            "TxPayload's canonical bincode encoding changed"
        );

        assert_eq!(
            tx.signing_hash().to_hex(),
            "93a824534f5c7226b927a1c312161a2d0f9ce97527606c6e26c0abf2b72bcc5d",
            "Transaction::signing_hash changed — update this AND client/src/services/helixTx.ts in Spark"
        );
    }
}
