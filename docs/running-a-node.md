...
| `HELIX_TRUSTED_CHECKPOINT` | (unset — replay from genesis) | `<height>:<block hash>`, e.g. `170000:141c8e0f…`. Any node will hand you a current one at `GET /sync/checkpoint`, and the status page shows it — **ask more than one and compare**, because a checkpoint from the node you are about to sync from proves nothing on its own. Lets a **new** node start from a verified state snapshot instead of replaying every block — minutes instead of hours. **You have to get this value from outside the network**, because a joining node has no validator set until it has a state, and the state is the thing being checked; that is the same requirement Ethereum calls weak subjectivity. Take it from a source you trust — the project's release notes, the explorer, an operator you know — and never from the node you are about to sync from. The height must be one the network snapshots (a multiple of `HELIX_SNAPSHOT_INTERVAL`, default 10000). Only used when this node holds no chain yet; a node with blocks of its own always keeps them. If anything about the proof fails the node says so and replays from genesis instead — the shortcut is never taken on faith. |
