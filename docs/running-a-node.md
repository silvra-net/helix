...
| `MALLOC_ARENA_MAX` | (unset — set it to `2`) | **Not a Helix variable, and the single cheapest thing you can do for a long-running node.** glibc gives every thread that allocates concurrently its own heap arena — up to 8 x your core count, each able to grow to 64 MiB — and memory freed inside an arena stays there rather than going back to the operating system. Nothing leaks; RSS simply climbs and never falls. Measured on the live chain on 2026-09-08, same binary, only this variable differing: without it 106 MB at one minute, 490 MB at five and still rising, **3177 MB after fourteen hours**; with `MALLOC_ARENA_MAX=2`, RSS settled at **256 MB and stayed there**. On a 1 GB machine the difference is whether your node survives the night — and an out-of-memory kill leaves nothing in the node's own log (see "When your node keeps stopping"). Two rather than one, because a single arena serialises every allocation behind one lock. |
