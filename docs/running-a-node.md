...
| `HELIX_GENESIS_HASH` | the public chain's genesis, compiled in | Hex hash of the genesis block you expect to join, checked against whatever the sync peer serves **before** anything is written — a mismatch aborts startup instead of adopting the wrong chain. **Joining the public seed is already checked without setting anything**, against a hash built into the binary. Set this to join a different network, or to get past a build whose compiled-in hash predates a chain reset (see "Verifying which chain you joined"). When joining, ignored with `HELIX_NEW_CHAIN` (that node signs its own genesis) and not applied when the genesis arrives over P2P from seed peers rather than from a named sync peer. **A node that already holds a chain is checked as well, at every start:** if its database belongs to another chain than this value — or, on the public seed, than the compiled-in hash — it refuses to start and names the rename (see "Verifying which chain you joined"). Overrides `genesis_hash` in `helix.toml`. |
...
| `HELIX_RPC_RATE_LIMIT` | `500,100` | Request budget per client IP as `burst,refill_per_sec`. Deliberately set above what the chain itself can absorb (~180 tx/s at a 2 MB block every 2 s), so the limit a submitter actually meets is block capacity, the mempool cap and the rising base fee — not this bucket. It was `30,10` until 2026-09-01, which silently capped submission an order of magnitude below block capacity: a 422-transaction batch got ~70 in and the rest a 429. Lower it if you expose a node to untrusted traffic and would rather shed load early. Malformed values are ignored with a warning; the node still starts on the default. |
...
| `HELIX_DB_CACHE_MB` | `128` | Page cache the embedded database may hold, in MiB. Sized so the cache itself is never the thing that pushes a node over. It is not the whole story on total memory: see `MALLOC_ARENA_MAX` below, and budget from the plateau measured there rather than from this number. Raise it (e.g. `512`) on a server with memory to spare and RPC read traffic to serve; there is no reason to lower it. |
| `MALLOC_ARENA_MAX` | (unset — set it to `2`) | **Not a Helix variable, and the single cheapest thing you can do for a long-running node.** glibc gives every thread that allocates concurrently its own heap arena — up to 8 x your core count, each able to grow to 64 MiB — and memory freed inside an arena stays there rather than going back to the operating system. Nothing leaks; RSS simply climbs and never falls. Measured on the live chain on 2026-09-08, same binary, only this variable differing: without it 106 MB at one minute, 490 MB at five and **still climbing to 3177 MB after fourteen hours**; with `MALLOC_ARENA_MAX=2` it stops climbing and settles. **How high it settles depends on the node, and the first measurement of that was too short.** It was taken over eight minutes and read 256 MB; re-measured on 2026-09-09 on a validator carrying a 12.8 GB chain database, six validators and live RPC traffic, the plateau is **about 1030 MB**, and it is a real one: across **15.3 hours** of observation it moved by 260 KB, or 0.02%. What the variable buys is the *flat* part, which is the part that matters: unbounded growth ends in an out-of-memory kill, and that kill leaves nothing in the node's own log (see "When your node keeps stopping"). Where exactly your own node settles is not something this document can tell you — watch `rss_kb` in `/diagnostics` over a day and size the machine above what you see, not above what is written here. A 1 GB machine was comfortable for a follower on a small chain and is **not** enough for a validator on a chain this size. Two rather than one, because a single arena serialises every allocation behind one lock. |
| `HELIX_TRUSTED_CHECKPOINT` | (unset — replay from genesis) | `<height>:<block hash>:<state root>`, e.g. `170000:141c8e0f…:9b2d…`. Any node will hand you a current one at `GET /sync/checkpoint` — **ask more than one and compare**, because a checkpoint from the node you are about to sync from proves nothing on its own. Lets a **new** node start from a verified state snapshot instead of replaying every block — minutes instead of hours. **The third part is what makes the shortcut possible at all.** A joining node cannot prove a state from anything the serving peer also controls: the block above the anchor carries a state commitment, but nothing stops a peer from writing that block itself, and checking a quorum on it would mean weighing signatures against a validator set read out of the very snapshot under test. So the state root has to reach the node from outside, exactly the way the genesis hash does — that is what Ethereum calls weak subjectivity. **A two-part checkpoint still works** and still anchors the blocks; it just does not license the snapshot shortcut, and the node says so and replays from genesis instead. The height must be one the network snapshots (a multiple of `HELIX_SNAPSHOT_INTERVAL`, default 10000). Only used when this node holds no chain yet; a node with blocks of its own always keeps them. If anything about the proof fails the node says so and replays from genesis instead — the shortcut is never taken on faith. **A value the node cannot read stops it at startup**, with the reason — "the block hash has 63 characters, and a block hash has 64" — instead of being ignored: ignoring it meant replaying the whole chain onto a disk that may be too small for it, which is how a validator lost its database in September 2026. Check a copied hash with `echo -n <hash> \| wc -c` (64). |
| `HELIX_SNAPSHOT_INTERVAL` | `10000` | How often this node stores a state snapshot, in heights. These are the fixed points another node starts from instead of replaying the chain, served by `GET /sync/snapshot?height=N`. A node holds only one *live* state — its current one — so without stored snapshots it could only ever offer the tip, and a tip moves while the joiner is verifying it. Cheap: about 1 KB per account, so single-digit megabytes against the gigabytes of blocks they replace. Pruning drops old ones but always keeps the newest three. `0` turns them off — then this node cannot help anyone bootstrap, which is the only reason to set it. Anything unparseable falls back to the default rather than disabling them. |
| `HELIX_KEEP_BLOCKS` | (unset — keep everything) | Number of recent blocks this node keeps on disk. Unset means **archive**: every block since genesis, which is what a network needs at least some nodes to do — and why it is not a default that discards. Set it (e.g. `50000`, about a day and a half at the 2 s cadence — digits only, no separators or units) and the node drops older blocks a batch at a time, keeping disk growth bounded. **A value the node cannot read stops it at startup** (`500_000`, `500k`): this limit is what keeps the disk from filling, and it used to be taken for "unset" instead. Values below 1000 are raised to 1000: consensus itself needs only the tip, but a peer that fell behind catches up by asking somebody for those blocks, and a node keeping fifty helps nobody. Genesis is never dropped. **It applies while the node catches up, too** — a node joining from block 0 used to write the whole chain before its window did anything (a validator on a 96 GB disk reached 17 GB that way), and now prunes as it syncs. **The database file will not shrink.** redb does not return space to the filesystem and its `compact()` gave nothing back when measured — what pruning buys is that later blocks reuse the freed pages instead of extending the file: measured, writing 46 MB after pruning 46 MB grew the file by 11 MB where the same writes without pruning cost 75 MB. So watch the growth rate, not `ls`. A pruning node can no longer serve the history it dropped, to peers or to its own wallet queries; `/diagnostics` reports `earliest_block` so a client can tell. |
| `HELIX_KEEP_BYTES` | (unset — no budget) | A disk budget for the chain database instead of a block count: `120G`, `120GiB`, `120 GB`, `4096M`, `2T`, or plain bytes — a whole number, any case; every unit is binary (`1G` = 1024³ bytes) however it is written. **A value the node cannot read stops it at startup** (`1.5G`, `120 gigs`) instead of being taken for "no budget", as it used to be. The node measures what its retained blocks actually cost on disk and derives the window from that, so it holds however much history fits — where `HELIX_KEEP_BLOCKS` holds a fixed count whose size depends on how full the blocks turn out to be. **That difference is the reason this exists:** 500,000 blocks is about 119 GB while blocks stay at 256 KB, and 887 GB if they run full at 2 MB. A byte budget does not move when traffic does. **Ask for less than the power of two you mean.** redb extends its file in doubling steps and never returns space, so content just over 64 GB occupies a 128 GB file. Measured end to end: an 80 MB budget settled at a 129 MB file, pruning 1764 blocks down to 1099. `120G` is the right way to say "never more than 128 GB"; `128G` is not, because it leaves the content free to cross 128 and take the file to 256. The `MIN_KEEP_BLOCKS` floor of 1000 blocks wins over the budget — a node keeping fifty would still run consensus and be useless to every peer, and a budget that cannot hold 1000 blocks is a disk problem this loop cannot fix by making the node a worse neighbour. Set both this and `HELIX_KEEP_BLOCKS` and the tighter of the two applies. |
| `HELIX_P2P_PUBLIC_ADDR` | (none) | This node's own externally-dialable address, announced to peers via peer exchange (see "Network Resilience" below). Either a bare host (a domain or public IP, no scheme/port — the configured raw-TCP P2P port is appended automatically), or, for a node behind a proxy/tunnel, a full multiaddr starting with `/` (e.g. `/dns4/host/tcp/443/tls/ws`). **An address no peer could dial stops the node at startup** — one that does not parse, or a bare host with a port stuck to it (`203.0.113.7:8546`; the message shows the full form). Overrides `p2p_public_addr` in `helix.toml`. Leave unset for followers with no public/forwarded port — they still relay addresses they learn from others. |
| `HELIX_P2P_SEED_PEERS` | (none) | Comma-separated libp2p multiaddrs (e.g. `/ip4/1.2.3.4/tcp/8546,/dns4/peer.example/tcp/8546`) to dial directly, in addition to the one derived from `sync_peer`. With no local chain and no `sync_peer`, these are also where the genesis block is fetched from — see "Joining without an HTTP endpoint". Use this to wire a validator set into a full mesh — every validator should peer with every other, not hub-and-spoke through one node. **An entry that is not a multiaddr stops the node at startup** and the message names the one it meant — `203.0.113.7:8546` is `/ip4/203.0.113.7/tcp/8546`. (It used to be dropped without a word.) Overrides `p2p_seed_peers` in `helix.toml`. |
| `HELIX_MAX_PROPOSAL_BYTES` | `262144` (256 KB) | Transaction bytes this node packs into blocks **it proposes**. Local policy, not a consensus rule — the protocol maximum is unchanged and every node still accepts blocks up to it. It exists because gossip has to reach every validator inside the proposal window, and on a network where peers reach each other through one relay that window is spent on bandwidth: measured on the live chain, 4 MB of blocks served locally in 0.023 s took 17.8 s through the tunnel every peer connects by. A proposal of size S then costs roughly `4·S / 500 KB` seconds to fan out, and a proposal that misses enough validators in time splits their prevotes and loses the round — 16 seconds, during which more transactions queue and the next block is larger still. Raise it if your validators peer directly with each other rather than through a relay; values above the protocol maximum are clamped, not obeyed. |
...
| `HELIX_PERSONHOOD_AUTHORITIES` | (none) | Comma-separated hex public keys allowed to issue Proof-of-Personhood attestations. Only takes effect at genesis — an existing chain's authorities were fixed when it was created. Unset means personhood attestations are disabled on this chain: a node refuses `ProvePersonhood` transactions at its RPC and from peers, and a block that carries one fails it without reading its proof. |
...
**The same check runs on the chain you already have.** After a reset, the release notes ask you to
rename your data directory. A node that still holds the old chain used to load it and run on it —
its sync failing on the first block that does not chain, its peers on another chain, nothing on
screen saying so. Now, if it knows which chain it is meant to be on (the compiled-in hash on the
public seed, or your `HELIX_GENESIS_HASH`), it refuses to start, names both hashes and the rename:
...
Keep `validator-key.json` — the key carries over, only the chain is new. In the desktop wallet:
**Validate → Reset local chain**. Nothing is written before the refusal. A node on its own
chain (`HELIX_NEW_CHAIN`) or behind your own `HELIX_SYNC_PEER` without a pinned hash has no such
expectation and is not checked.
...
2. **Then the machine.** A reboot, a hypervisor migration or a full disk all look the same from
   inside the process. A node filling its own disk now stops before the disk is full: every
   block, state and receipt write needs **1 GiB free** on the disk that holds the database, and
   below that the node refuses the write, logs "only … MB are free on the disk that holds the chain
   database" and exits. It will keep exiting on restart until there is room — that is the point:
   a write that runs out of space half-way leaves a database no build can open, which is what
   happened to a validator whose disk filled in September 2026. Free space, or bound the database
   with `HELIX_KEEP_BLOCKS` / `HELIX_KEEP_BYTES`. (Linux and macOS; on Windows the reserve cannot
   be read and is not enforced.)
