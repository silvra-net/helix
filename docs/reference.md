...
| GET | `/sync/checkpoint` | A height and block hash to anchor a fast join to, in the exact shape `HELIX_TRUSTED_CHECKPOINT` takes (`{"height":10000,"block_hash":"…","checkpoint":"10000:…"}`). **An answer, not a proof:** a checkpoint taken from the node you are about to sync from is the same party vouching for itself. It is worth something when it is *compared* — every node answers this, so ask two or three and see whether they agree. Names only a height where a join would actually succeed: the stored snapshot, the block at that height, and the block above it must all still be here. 404 while none is, which is normal on a young or freshly pruned node. |
