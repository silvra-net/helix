...
| GET | `/sync/snapshot` | The whole chain state plus the height it belongs to, as `application/octet-stream` (bincode only — nobody reads a snapshot, and JSON would cost 4.5x for nothing). Groundwork for starting a node from a state instead of replaying every block: the receiver hashes what it gets and compares against `prev_state_root` in the header of the *next* block, which is signed. That verifies the state against the chain rather than against the server that served it — but it still needs the block it compares to be genuine, which is an out-of-band checkpoint the operator supplies (what Ethereum calls weak subjectivity). No client uses this yet. |
